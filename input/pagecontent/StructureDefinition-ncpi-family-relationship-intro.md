...
Family relationship profiles can be constructed for each individual to reflect the different directions of relationships. For example, if we have a mother and son enrolled in a given research study, NCPI Family Relationship can be used to define how mother (`relative`) relates to son (`patient`) and what kind of code describes their relationship (`relationship`), in this case, the code "NPRN" for "natural parent." Use of multiple NCPI Family Relationship profiles to define relevant parties will as a whole describe interrelationships.
